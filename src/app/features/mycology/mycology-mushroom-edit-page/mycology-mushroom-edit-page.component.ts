...
export class MycologyMushroomEditPageComponent implements OnChanges {
constructor(private dataService: DataService, private store: Store<MycologyState>){}
 @Input() set id(mushroomId: number) {
  ...
  onUpdate(mushroom: Mushroom) {
    this.store.dispatch(
      MushroomsActions.updateMushroomRequest({...mushroom, id: this.mushroomID})
    );
  }
}
